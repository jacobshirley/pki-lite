...
import { PFX } from '../../src/pkcs12/PFX'
...
-----END PKCS12-----`
        ...
        expect(privateKeys[0].getPrivateKey().toPem()).toMatchSnapshot()

        const certificates = await pfx.getX509Certificates('test')
        expect(certificates.length).toBe(1)
        expect(certificates[0].toPem()).toMatchSnapshot()
    })
})
